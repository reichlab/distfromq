...
to a distribution obtained by interpolating and extrapolating from a set
...
a function with argument `n` that can be used to generate a sample of
...
quantile function is then obtained by inverting this estimate of the
CDF.
...
with argument `p` that evaluates the quantile function.
