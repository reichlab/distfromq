...
a function with arguments `q` and `log.p` that can be used to evaluate
the approximate cumulative distribution function (or its `log`) at the
points `q`.
...
interpolates the provided `(q, p)` pairs as an estimate of the CDF.
