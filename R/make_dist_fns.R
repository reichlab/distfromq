...
make_p_fn <- function(ps, qs,
    ...
    if (disc_weight < 1.0) {
        ...
        interior_args <- c(
            list(ps = cont_ps, qs = cont_qs, tail_dist = tail_dist,
                 fn_type = "p"),
            interior_args)
        ...
        # lowest quantiles within a location-scale family
        if (min(cont_ps) > 0.0) {
            lower_cdf <- p_ext_factory(head(cont_ps, 2), head(cont_qs, 2),
                                       tail_dist)
        }
        ...
        # largest quantiles within a location-scale family
        if (max(cont_ps) < 1.0) {
            upper_cdf <- p_ext_factory(tail(cont_ps, 2), tail(cont_qs, 2),
                                       tail_dist)
        }
    }
...
make_q_fn <- function(ps, qs,
    ...
    if (disc_weight < 1.0) {
        ...
        if (max(cont_ps) < 1.0) {
            upper_qf <- q_ext_factory(tail(cont_ps, 2), tail(cont_qs, 2),
                                      tail_dist)
        }
    }
